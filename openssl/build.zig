...
pub fn build(b: *std.Build) void {
    ...
    const ssl_dir_flag: []const u8 = switch (target.result.os.tag) {
    ...
    // x86_64 ASM flags
    const x86_64_asm_flags = [_][]const u8{
        "-DAES_ASM",
        "-DBSAES_ASM",
        "-DCMLL_ASM",
        "-DECP_NISTZ256_ASM",
    ...
    // macOS flag to disable CommonCrypto
    const darwin_flags = [_][]const u8{
        "-DOPENSSL_NO_APPLE_CRYPTO_RANDOM",
    };

    // Only use x86_64 ASM flags on Linux (where we have the assembly files)
    const use_x86_64_asm_flags = is_x86_64 and is_linux;

    // Build flag slices based on target
    const base_flags: []const []const u8 = if (use_x86_64_asm_flags)
        &(common_base_flags ++ x86_64_asm_flags)
    else if (is_darwin)
        &(common_base_flags ++ darwin_flags)
    else
        &common_base_flags;
    ...
    mod.addCSourceFiles(.{
        ...
        .files = &provider_gen_files,
        .flags = base_flags,
    });

    // Crypto sources - common files
    mod.addCSourceFiles(.{
        .root = upstream.path("crypto"),
        .files = &crypto_common_files,
        .flags = base_flags,
    });
    ...
    const use_x86_64_asm = is_x86_64 and is_linux;
    ...
    if (use_x86_64_asm) {
        ...
        // x86_64 assembly files from upstream (which has pre-generated .s files)
        mod.addCSourceFiles(.{
            .root = b.path("crypto"),
            .files = &x86_64_asm_files,
            .flags = base_flags,
        });
    } else {
        // Non-x86_64-Linux: use C fallback implementations
        mod.addCSourceFiles(.{
            .root = upstream.path("crypto"),
            .files = &.{
                "aes/aes_core.c",
    ...
    mod.addIncludePath(b.path("crypto")); // For arm_arch.h
...
const provider_files = [_][]const u8{
    "common/bio_prov.c",
...
const provider_gen_files = [_][]const u8{
...
const x86_64_asm_files = [_][]const u8{
    "aes/aes-x86_64.s",
...
// Crypto common files (excluding x86_64-specific and Linux-specific)
const crypto_common_files = [_][]const u8{
    "aes/aes_cbc.c",
    "aes/aes_cfb.c",
    "aes/aes_ecb.c",
    "aes/aes_ige.c",
    ...
    "mdc2/mdc2dgst.c",
    "mem.c",
    // mem_clr.c is conditionally added - on x86_64 Linux it's in x86_64cpuid.s
    "mem_sec.c",
    "modes/cbc128.c",
    "modes/ccm128.c",
    "modes/cfb128.c",
